...
class TaskBalancedSampler(torch.utils.data.Sampler):
    """Task-uniform sampler for a ConcatDataset of task datasets.

    The epoch keeps the original total number of samples, but assigns them
    round-robin across child datasets. Within each child, independently shuffled
    indices are cycled as needed. Accelerator performs process sharding later.
    """

    def __init__(self, dataset, seed=0, num_samples=None):
        if not isinstance(dataset, torch.utils.data.ConcatDataset):
            raise TypeError("task-balanced sampling requires torch.utils.data.ConcatDataset")
        self.dataset = dataset
        self.lengths = [len(child) for child in dataset.datasets]
        if not self.lengths or any(length <= 0 for length in self.lengths):
            raise ValueError(f"all task datasets must be non-empty, got {self.lengths}")
    ...
    def __iter__(self):
        ...
        task_count = len(self.lengths)
        orders = [torch.randperm(length, generator=generator).tolist() for length in self.lengths]
        cursors = [0] * task_count
        task_order = torch.randperm(task_count, generator=generator).tolist()
        result = []
        for position in range(self.num_samples):
            if position and position % task_count == 0:
                task_order = torch.randperm(task_count, generator=generator).tolist()
            task_index = task_order[position % task_count]
            if cursors[task_index] >= len(orders[task_index]):
                orders[task_index] = torch.randperm(
                    self.lengths[task_index], generator=generator
                ).tolist()
                cursors[task_index] = 0
            local_index = orders[task_index][cursors[task_index]]
            cursors[task_index] += 1
            result.append(self.offsets[task_index] + local_index)
        return iter(result)


class TaskNamedDataset(torch.utils.data.Dataset):
    """Attach a diagnostics-only source name without feeding task IDs to Tau."""
    ...
    def __getitem__(self, index):
        ...
        sample["monitor_task_name"] = self.task_name
        sample["monitor_task_local_index"] = int(index)
        return sample
...
class Trainer:
    ...
    def prepare_dataset_new(self, dataset_configs, public_args):
        ...
        for dataset_config in dataset_configs:
            ...
            if getattr(self.args, "task_balanced_sampling", False):
                _dataset = TaskNamedDataset(_dataset, Path(dataset_config).stem)
    ...
    def prepare_dataset(self) -> None:
        ...
        loader_args = self.get_loader_args()
        if getattr(self.args, "task_balanced_sampling", False):
            loader_args["sampler"] = TaskBalancedSampler(
                self.train_dataset,
                seed=getattr(self.args, "seed", 0) or 0,
                num_samples=getattr(self.args, "task_balanced_epoch_samples", None),
            )
    ...
    def train(self):
        ...
        # loss spikes
        anomalies = []
        monitor_enabled = bool(getattr(self.args, "multitask_training_log", None))
        monitor_started = time.monotonic()
        monitor_steps = []
        monitor_task_counts = {}
        monitor_task_loss_sums = {}
        monitor_nonfinite = 0
        ...
        for epoch in range(first_epoch, self.state.train_epochs):
            ...
            for step in range(min_steps_per_epoch):
                ...
                raw_task_names = [
                    str(value)
                    for value in batch.get('monitor_task_name', batch.get('caption', []))
                ]
                raw_task_local_indices = [
                    int(value)
                    for value in (batch.get('monitor_task_local_index') or [])
                ]
                with accelerator.accumulate([ self.diffusion_model ]):
                    ...
                    if not bool(torch.isfinite(loss)):
                        monitor_nonfinite += 1
                    assert torch.isfinite(loss), "NaN/Inf loss detected"
                ...
                telemetry_path = getattr(self.args, "reproduction_telemetry_log", None)
                if (
                    telemetry_path
                    and accelerator.is_main_process
                    and global_step > 0
                    and (global_step % 25 == 0 or (75 <= global_step <= 300 and global_step % 5 == 0))
                ):
                    gpu_query = subprocess.run(
                        [
                            "nvidia-smi", "-i", os.environ.get("CUDA_VISIBLE_DEVICES", "0").split(",")[0],
                            "--query-gpu=uuid,memory.used,memory.total,utilization.gpu,temperature.gpu",
                    ...
                    gpu_fields = [value.strip() for value in gpu_query.stdout.strip().split(",")]
                    telemetry_record = {
                        "timestamp_utc": datetime.utcnow().isoformat(timespec="seconds") + "Z",
                        "global_step": global_step,
                        "loss": float(loss.item()),
                        "loss_video": float(loss_video.item()) if self.args.return_video else None,
                        "loss_action": float(loss_action.item()) if self.args.return_action else None,
                        "gpu_allocated_bytes": int(torch.cuda.memory_allocated()) if torch.cuda.is_available() else 0,
                        "gpu_reserved_bytes": int(torch.cuda.memory_reserved()) if torch.cuda.is_available() else 0,
                        "gradient_finite": grad_finite,
                        "task_identity": raw_task_names,
                        "task_local_index": raw_task_local_indices,
                        "gpu_query_returncode": gpu_query.returncode,
                        "gpu_uuid": gpu_fields[0] if len(gpu_fields) == 5 else None,
                        "gpu_memory_used_mib": int(gpu_fields[1]) if len(gpu_fields) == 5 else None,
                        "gpu_memory_total_mib": int(gpu_fields[2]) if len(gpu_fields) == 5 else None,
                        "gpu_utilization_percent": int(gpu_fields[3]) if len(gpu_fields) == 5 else None,
                        "gpu_temperature_c": int(gpu_fields[4]) if len(gpu_fields) == 5 else None,
                    }
                    telemetry_file = Path(telemetry_path)
                    telemetry_file.parent.mkdir(parents=True, exist_ok=True)
                    with telemetry_file.open("a", encoding="utf-8") as handle:
                        handle.write(json.dumps(telemetry_record, sort_keys=True) + "\n")
                        handle.flush()

                if monitor_enabled:
                    for task_name in raw_task_names:
                        monitor_task_counts[task_name] = monitor_task_counts.get(task_name, 0) + 1
                        monitor_task_loss_sums[task_name] = (
                            monitor_task_loss_sums.get(task_name, 0.0) + float(loss.item())
                        )
                    reduced = torch.tensor(
                        [
                            float(loss.item()),
                            float(loss_video.item()) if self.args.return_video else 0.0,
                            float(loss_action.item()) if self.args.return_action else 0.0,
                        ],
                        device=accelerator.device,
                        dtype=torch.float64,
                    )
                    dist.all_reduce(reduced, op=dist.ReduceOp.SUM)
                    reduced /= accelerator.num_processes
                    if accelerator.is_main_process:
                        monitor_steps.append(
                            {
                                "step": global_step,
                                "loss": float(reduced[0].item()),
                                "loss_video": float(reduced[1].item()),
                                "loss_action": float(reduced[2].item()),
                                "gpu_memory_allocated_bytes": int(torch.cuda.memory_allocated()) if torch.cuda.is_available() else 0,
                            }
                        )
        ...
        accelerator.wait_for_everyone()
        if monitor_enabled:
            def _invalid_windows(dataset):
                if isinstance(dataset, torch.utils.data.ConcatDataset):
                    return sum(_invalid_windows(child) for child in dataset.datasets)
                if isinstance(dataset, TaskNamedDataset):
                    return _invalid_windows(dataset.dataset)
                return len(getattr(dataset, "skip_idx", []))

            local_monitor = {
                "rank": accelerator.process_index,
                "task_sampling_counts": monitor_task_counts,
                "task_loss_sums": monitor_task_loss_sums,
                "invalid_windows": _invalid_windows(self.train_dataset),
                "nonfinite_loss_count": monitor_nonfinite,
                "peak_gpu_memory_allocated_bytes": int(torch.cuda.max_memory_allocated()) if torch.cuda.is_available() else 0,
                "peak_gpu_memory_reserved_bytes": int(torch.cuda.max_memory_reserved()) if torch.cuda.is_available() else 0,
            }
            gathered_monitors = [None for _ in range(accelerator.num_processes)]
            dist.all_gather_object(gathered_monitors, local_monitor)
            if accelerator.is_main_process:
                combined_counts = {}
                combined_loss_sums = {}
                for rank_monitor in gathered_monitors:
                    for caption, count in rank_monitor["task_sampling_counts"].items():
                        combined_counts[caption] = combined_counts.get(caption, 0) + count
                    for caption, value in rank_monitor["task_loss_sums"].items():
                        combined_loss_sums[caption] = combined_loss_sums.get(caption, 0.0) + value
                per_task_loss = {
                    caption: combined_loss_sums[caption] / combined_counts[caption]
                    for caption in combined_counts
                }
                losses = [step_record["loss"] for step_record in monitor_steps]
                monitor_payload = {
                    "schema": "tau-multitask-sft-training-health-v1",
                    "checkpoint_directory": os.path.join(self.save_folder, f"step_{global_step}"),
                    "training_steps": global_step,
                    "world_size": accelerator.num_processes,
                    "task_balanced_sampling": bool(getattr(self.args, "task_balanced_sampling", False)),
                    "task_sampling_counts_by_task": combined_counts,
                    ...
                    "step_log": monitor_steps,
                    "invalid_windows": sum(item["invalid_windows"] for item in gathered_monitors),
                    "nonfinite_loss_count": sum(item["nonfinite_loss_count"] for item in gathered_monitors),
                    "peak_gpu_memory_allocated_bytes_by_rank": {
                        str(item["rank"]): item["peak_gpu_memory_allocated_bytes"]
                        for item in gathered_monitors
                    },
                    "peak_gpu_memory_reserved_bytes_by_rank": {
                        str(item["rank"]): item["peak_gpu_memory_reserved_bytes"]
                        for item in gathered_monitors
                    },
                    "walltime_sec": round(time.monotonic() - monitor_started, 6),
                }
                monitor_path = Path(self.args.multitask_training_log)
                monitor_path.parent.mkdir(parents=True, exist_ok=True)
                monitor_tmp = monitor_path.with_suffix(monitor_path.suffix + ".tmp")
                monitor_tmp.write_text(json.dumps(monitor_payload, indent=2, ensure_ascii=False) + "\n")
                monitor_tmp.replace(monitor_path)
            accelerator.wait_for_everyone()
