#!/usr/bin/env python3
"""PB-B2: Closed-loop evaluation (canonical stats + layout).

Runs in robotwin env (SAPIEN). Spawns VAM inference subprocess in tau0_wm env.
Evaluates PB-B2 checkpoints step_100, step_300, step_802 (≈800) on seeds 100-104
with the CANONICAL statistics (statistics_relative_v2.json, SHA-256[:16] 0eda42bdbd2d43d7)
and the official native UniPC solver (vam_server -> TauPolicy.play default sample_solver='unipc',
NOT Flow-GRPO SDE).

Usage:
    cd ${ROBOTWIN_ROOT} && DISPLAY=:99 CUDA_VISIBLE_DEVICES=0 \
    /opt/conda/envs/robotwin/bin/python ${CAUSALWAM_ROOT}/eval/pbb2_closed_loop.py \
        --single step100
"""
...
OUTPUT_DIR = os.path.join(CAUSAL_ROOT, "outputs/pbb2_canonical/turn_switch/eval/closed_loop")
...
# Retry a VAM inference this many times (restarting the subprocess) before giving
# up, to ride out the intermittent "misaligned address" / "unspecified launch
# failure" GPU faults on the post-reboot driver.
VAM_MAX_RETRIES = 3
...
CHECKPOINTS = {
    "step100": {
        "path": os.path.join(CAUSAL_ROOT, "outputs/pbb2_canonical/turn_switch/2026_08_13_02_55_01/step_100"),
        "action_type": "relative",
    },
    "step300": {
        "path": os.path.join(CAUSAL_ROOT, "outputs/pbb2_canonical/turn_switch/2026_08_13_02_55_01/step_300"),
        "action_type": "relative",
    },
    "step802": {
        "path": os.path.join(CAUSAL_ROOT, "checkpoints/pbb2_turn_switch/step_802"),
        "action_type": "relative",
    },
}
...
# GPU separation: the parent driver (SAPIEN/Vulkan renderer + Warp physics) runs on
# the GPU selected by CUDA_VISIBLE_DEVICES at launch; the VAM inference subprocess
# (5.5B model) is pinned to a DIFFERENT physical GPU to avoid Vulkan+CUDA contention
# that intermittently faults the post-reboot driver ("unspecified launch failure" /
# "vk::DeviceLostError"). Override via env var PBB2_VAM_GPU (physical index).
VAM_GPU = os.environ.get("PBB2_VAM_GPU", "1")
...
def run_episode(checkpoint_name, checkpoint_info, seed, output_dir):
    ...
    try:
        for step in range(MAX_POLICY_STEPS):
            ...
            # Get action from VAM (with subprocess-restart retry on intermittent GPU fault).
            # The VAM subprocess reports internal errors (e.g. "misaligned address") as a
            # dict {"error": ...} rather than raising, so treat both as retryable.
            action_data = None
            for attempt in range(VAM_MAX_RETRIES):
                try:
                    action_data = vam.infer(obs)
                    if action_data.get("error"):
                        raise RuntimeError(action_data["error"])
                    break
                except Exception as e:
                    print(f"  Step {step}: VAM error (attempt {attempt+1}/{VAM_MAX_RETRIES}): {e}")
                    action_data = None
                    if attempt < VAM_MAX_RETRIES - 1:
                        try:
                            vam.close()
                        except Exception:
                            pass
...
def main():
    ...
    parser.add_argument("--checkpoints", nargs="+",
                        default=["step100", "step300", "step802"])
