#!/usr/bin/env python3
"""PB-B2: Interaction Snapshot Productivity Test (canonical stats + layout).

Selects I0/I1/I2 snapshots from PB-A trajectory replay,
generates K=4 action candidates per checkpoint×snapshot,
executes each in real environment, computes G_true = Y_i - Y0 (post - pre switch qpos).

Uses CANONICAL statistics (statistics_relative_v2.json) and PB-B2 checkpoints.
No ACVS this phase.

Usage:
    cd ${ROBOTWIN_ROOT} && DISPLAY=:99 PYTHONUNBUFFERED=1 \
    /opt/conda/envs/robotwin/bin/python ${CAUSALWAM_ROOT}/eval/pbb2_productivity.py
"""
...
# GPU separation: VAM inference subprocess pinned to a different physical GPU than the
# SAPIEN driver (same rationale as pbb2_closed_loop.py). Override via PBB2_VAM_GPU.
...
def main():
    ...
    parser.add_argument("--output-dir",
                        default=os.path.join(CAUSAL_ROOT, "outputs/pbb2_canonical/turn_switch/eval/productivity"))
    ...
    print("=" * 70)
    print("PB-B2 Phase E2: Interaction Snapshot Productivity Test")
    print(f"Task: {TASK}")
    ...
    # ── Summary ──
    print(f"\n{'='*70}")
    print("PB-B2 PRODUCTIVITY SUMMARY")
    print(f"{'='*70}")
